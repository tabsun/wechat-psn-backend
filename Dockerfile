...
RUN set -x \
	...
	&& python -m pip install --no-cache-dir --upgrade pip==$PYTHON_PIP_VERSION \
	&& find /usr/local \
...
# these maybe install
RUN python -m pip install --no-cache-dir pycrypto
RUN python -m pip install --no-cache-dir lxml
